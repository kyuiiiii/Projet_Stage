...
int way_callback(const void *user_data, const readosm_way *way) {
    ...
    for (int i = 0; i < way->node_ref_count - 1; i++) {
        ...
        if (oneway == 1) {
            addWay(n1, n2, vitesse, dist, road_name);
        } else if (oneway == -1) {
            addWay(n2, n1, vitesse, dist, road_name);
        } else {
            addWay(n1, n2, vitesse, dist, road_name);
            addWay(n2, n1, vitesse, dist, road_name);
        }
    ...
    for (int i = n - 1; i > 0; i--) {
        ...
        for (Way *e = graphe[from]; e; e = e->suiv) {
            ...
            if (e->to == to) {
                ...
                const char *road =
                    e->road_name ? e->road_name : "Unnamed road";

                if (strcmp(road, lastRoad) != 0)
                {
                    printf("\n=== %s ===\n", road);

                    strncpy(lastRoad, road, sizeof(lastRoad)-1);
                    lastRoad[sizeof(lastRoad)-1] = '\0';
                }
                printf(
                    "%s%s(%lld) -> %s%s(%lld) | %.0f m | %d km/h | %.1f s\n",
                    node_name[from] ? node_name[from] : "",
                    node_name[from] ? " " : "",
                    node_id[from],

                    node_name[to] ? node_name[to] : "",
                    node_name[to] ? " " : "",
                    node_id[to],
...
TempEdge connect_to_nearest_road(int node)
{
    ...
    if (graphe[nearest])
    {
        road_name = graphe[nearest]->road_name;
    }
    addWay(node, nearest, 5, best, road_name);
    addWay(nearest, node, 5, best, road_name);
